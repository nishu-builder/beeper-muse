...
| ACT-01  | Typing indicators do not show                                     | Live Matrix typing=true and false requests accepted. 0.8.4 broadens explicit source signals and adds source/transport diagnostics. Heartbeats and expiry implemented; client rendering unverified.         | **Open live verification.** Check the new source/transport logs and visible typing/clearing in both clients. Investigate room-feature advertisement without claiming unsupported outbound actions.   |
| ACT-02  | Reactions / acknowledgments rendered as message text or omitted   | Actor-aware source reactions and native reaction events implemented; not all live cases verified.                                                                                                          | **Open live verification.** Add/change/remove own and assistant reactions without text pollution; never convert reaction acknowledgments into read receipts.                                         |
...
| HIS-03  | Timestamps may be wrong                                           | Preserve machine-readable source timestamps; otherwise retain first observation time.                                                                                                                      | **Open.** Compare known source times/timezones and chronological ordering; never invent an original time.                                                                                            |
...
| Headings, quotes, code, tables                     | Not comprehensively inventoried                                                | Audit allowed formatting, syntax/newlines, table fallback and sanitization.                                                      |
...
| Edits / streaming updates                          | Observed                                                                       | Native edits implemented; verify final completeness, no duplicate notifications and image retention.                             |
